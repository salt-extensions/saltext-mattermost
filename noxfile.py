...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/mattermost/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/mattermost/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/mattermost/*"
            )
            ...
            #    "--include=src/saltext/mattermost/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/mattermost/config/schemas",
    )
